...
int main(int argc, char **argv) {
 ...
 //int windowSize = 48;          // par convention
 int windowSize = 512;
 //int length = (int)(sizeof(tableau) / sizeof(int));        // Nb échantillons dans wav_data = tableau
 //int length = 511;
 ...
 int samples = (int)(length/(windowSize/2))*((windowSize/2)+1);     // dimension de magnitude
 int hop_size = 512;            // facteur de recouvrement
 double *magnitude = malloc(sizeof(double)*samples);    // Sortie STFT
 ...
 int nCols = (int)(length/(windowSize/2));
 int nRows = (int)((windowSize/2)+1);
...
char *my_dtoa(double num, char *str){           // A refaire
        if(str == NULL)
        {
                return NULL;
        }
...
void dump_fichier_double(double *data, int size){
    ...
    printf("Fin dump_fichier double\n");
}
